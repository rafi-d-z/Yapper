    ...
    async function insertUserType() {
      try {
        const { data, error } = await supabase
          .from('user')
          .insert([
            { id: userID, user_name: username, user_type: filter }, 
          ]);
    ...
    return (
      <>
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-center border-slate-200 drop-shadow items-center">
          ...
          < UserSelection />
          <button className="w-11/12 h-8 bg-[#52C41A] text-white font-bold" style={{ marginTop: "20px" }} onClick={handleCreateAccount}>
              Sign Up
            </button>
